...
- Quick fallback without changing the system install:
  ...
  - `npx -y -p node@22 -p @steipete/oracle -c 'node $(readlink -f $(which oracle)) --dry-run summary --files-report -p "<task>" --file "src/**"'`
...
3. Run the local fork in browser mode with exact model-label selection; use API only when you explicitly want it.
...
- Preview (no tokens):
  - `node /home/lorenzo/oracle/dist/bin/oracle-cli.js --dry-run summary -p "<task>" --file "src/**" --file "!**/*.test.*"`
  - `node /home/lorenzo/oracle/dist/bin/oracle-cli.js --dry-run full -p "<task>" --file "src/**"`

- Token/cost sanity:
  - `node /home/lorenzo/oracle/dist/bin/oracle-cli.js --dry-run summary --files-report -p "<task>" --file "src/**"`

- Browser run (main path; long-running is normal):
  - `node /home/lorenzo/oracle/dist/bin/oracle-cli.js --engine browser --browser-manual-login --browser-port 9222 --browser-chrome-path /usr/bin/google-chrome --browser-model-strategy select --browser-model-label "GPT-5.5 Pro" --browser-thinking-time extended -p "<task>" --file "src/**"`
...
- Prefer attaching the real files with `--file` instead of flattening everything into one rendered text blob.
...
- When the input is a software worktree, try to preserve the worktree layout and relative paths so Oracle can reason about modules, tests, docs, and configs in context.
...
- When the issue spans a parent repo such as `moleworks_ros` plus sibling repos, preserve the repo boundaries and relative paths in the uploaded context.
- If a single module is not enough to explain the bug, it is acceptable to send a lightweight zip of the parent repo plus selected sibling repos and evidence files.
- The goal is not archival completeness. The goal is to give Oracle the smallest bundle that still contains the real truth.
...
- Stage a clean temporary directory that keeps the original repo names, for example `bundle/moleworks_ros`, `bundle/moleworks_ext`, `bundle/newton`.
- Copy only the relevant code and evidence into that staging area.
- Add a small `README.md` or `TREE.txt` at the bundle root describing the contents.
...
- Bottom: attach _all_ context files needed so a fresh model can fully understand (entrypoints, configs, key modules, docs, and distilled experimental evidence when relevant).

For Moleworks-style work, this often means:

- Attach a lightweight zip of the parent repo, for example `moleworks_ros`, when that is the real unit of context.
- Include relevant sibling repos in the same staged bundle when the behavior crosses repo boundaries.
- Keep the zip code-heavy and evidence-heavy, not artifact-heavy.
