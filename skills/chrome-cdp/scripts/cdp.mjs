...
const BROWSER_DAEMON_ID = '__browser__';
...
async function openStr(cdp, url) {
  ...
  if (!pages.some(p => p.targetId === targetId)) {
    pages.push({ targetId, title: targetUrl, url: targetUrl });
  }
  writeFileSync(PAGES_CACHE, JSON.stringify(pages), { mode: 0o600 });
  return `Opened new tab: ${targetId.slice(0, 8)}  ${targetUrl}
...
async function runBrowserDaemon() {
  const sp = sockPath(BROWSER_DAEMON_ID);
  ...
  async function handleCommand({ cmd, args }) {
    ...
    try {
      switch (cmd) {
        ...
        default:
          return { ok: false, error: `Unknown command for browser daemon: ${cmd}` };
      }
    } catch (e) {
...
async function getOrStartBrowserDaemon() {
  ...
  const child = spawn(process.execPath, [process.argv[1], '_daemon_browser'], {
    detached: true,
    stdio: 'ignore',
  });
  child.unref();
...
async function stopDaemons(targetPrefix) {
  if (!targetPrefix) {
    try {
      const browserConn = await connectToSocket(sockPath(BROWSER_DAEMON_ID));
      await sendCommand(browserConn, { cmd: 'stop' });
    } catch {
      if (!IS_WINDOWS) try { unlinkSync(sockPath(BROWSER_DAEMON_ID)); } catch {}
    }
  }
...
async function main() {
  ...
  if (cmd === '_daemon_browser') { await runBrowserDaemon(); return; }
  ...
  if (cmd === 'open') {
    const conn = await getOrStartBrowserDaemon();
    const response = await sendCommand(conn, { cmd: 'open', args: [args[0] || 'about:blank'] });
    if (response.ok) {
      if (response.result) console.log(response.result);
      return;
    }
